...
function importTxt($txt){
	...
	if (strpos($txt,"\r")!==false && strpos($txt,"<BR>")===false) // MAC
		$txt = str_replace("\r","<BR>",$txt);
	else // PC?
		$txt = str_replace("\r",'',$txt);
	$txt = $txt."<BR>";// makes things easier

	$headerStr = strtok($txt,"<BR>");
	$header = explode(' ',$headerStr); //"MFile $type $tc $timebase";
	$this->type = $header[1];
	$this->timebase = $header[3];
	$this->tempo = 0;

	$trackStrings = explode("MTrk<BR>",$txt);
	array_shift($trackStrings);
	$tracks = array();
	foreach ($trackStrings as $trackStr){
		$track = explode("<BR>",$trackStr);
		array_pop($track);
		array_pop($track);
...
function getTrackTxt($tn,$ttype=0){ //0:absolute, 1:delta
	...
	}else foreach ($track as $msg) $str .= $msg."<BR>";
	$str .= "TrkEnd<BR>";
	return $str;
}
...
<TimestampType>".($ttype==1?'Delta':'Absolute')."</TimestampType><BR>";

  for ($i=0;$i<$tc;$i++){
	$xml .= "<Track Number=\"$i\"><BR>";
	$track = $tracks[$i];
	$mc = count($track);
	$last = 0;
	for ($j=0;$j<$mc;$j++){
		...
		$xml .= "  <Event><BR>";
		$xml .= ($ttype==1)?"    <Delta>$dt</Delta><BR>":"    <Absolute>$t</Absolute><BR>";
		$xml .= '    ';
			...
			switch($msg[1]){
				case 'PrCh':
					eval("\$".$msg[2].';'); // $ch
					eval("\$".$msg[3].';'); // $p
				$xml .= "<ProgramChange Channel=\"$ch\" Number=\"$p\"/><BR>";
					break;
				...
				case 'Off':
					...
					eval("\$".$msg[3].';'); // $n
					eval("\$".$msg[4].';'); // $v
				$xml .= "<Note{$msg[1]} Channel=\"$ch\" Note=\"$n\" Velocity=\"$v\"/><BR>";
					break;
				...
				case 'PoPr':
					...
					eval("\$".$msg[3].';'); // $n
					eval("\$".$msg[4].';'); // $v
					$xml .= "<PolyKeyPressure Channel=\"$ch\" Note=\"$n\" Pressure=\"$v\"/><BR>";
					break;
				...
				case 'Par':
					...
					eval("\$".$msg[3].';'); // c
					eval("\$".$msg[4].';'); // v
					$xml .= "<ControlChange Channel=\"$ch\" Control=\"$c\" Value=\"$v\"/><BR>";
					break;

				case 'ChPr':
					eval("\$".$msg[2].';'); // ch
					eval("\$".$msg[3].';'); // v
					$xml .= "<ChannelKeyPressure Channel=\"$ch\" Pressure=\"$v\"/><BR>";
					break;

				case 'Pb':
					eval("\$".$msg[2].';'); // ch
					eval("\$".$msg[3].';'); // v
					$xml .= "<PitchBendChange Channel=\"$ch\" Value=\"$v\"/><BR>";
					break;

				case 'Seqnr':
					$xml .= "<SequenceNumber Value=\"{$msg[2]}\"/><BR>";
					break;

				case 'Meta':
					$txttypes = array('Text','Copyright','TrkName','InstrName','Ly  ric','Marker','Cue');
					$mtype = $msg[2];

					$pos = array_search($mtype, $txttypes);
					if ($pos !== FALSE){
						$tags = array('TextEvent','CopyrightNotice','TrackName','I  nstrumentName','Lyric','Marker','CuePoint');
						$tag = $tags[$pos];
						$line = $track[$j];
						$start = strpos($line,'"')+1;
						$end = strrpos($line,'"');
						$txt = substr($line,$start,$end-$start);
						$xml .= "<$tag>".htmlspecialchars($txt)."</$tag><BR>";
					}else{
						if ($mtype == 'TrkEnd')
						$xml .= "<EndOfTrack/><BR>";
						elseif ($mtype == '0x20' || $mtype == '0x21') // ChannelPrefix ???
						$xml .= "<MIDIChannelPrefix Value=\"{$msg[3]}\"/><BR>";
					}
					break;

				case 'Tempo':
					$xml .= "<SetTempo Value=\"{$msg[2]}\"/><BR>";
					break;

				case 'SMPTE':
					$xml .= "<SMPTEOffset TimeCodeType=\"1\" Hour=\"{$msg[2]}\" Minute=\"{$msg[3]}\" Second=\"{$msg[4]}\" Frame=\"{$msg[5]}\" FractionalFrame=\"{$msg[6]}\"/><BR>"; //TimeCodeType???
					break;

				case 'TimeSig': // LogDenum???
					$ts = explode('/',$msg[2]);
				$xml .= "<TimeSignature Numerator=\"{$ts[0]}\" LogDenominator=\"".log($ts[1])/log(2)."\" MIDIClocksPerMetronomeClick=\"{$msg[3]}\" ThirtySecondsPer24Clocks=\"{$msg[4]}\"/><BR>";
					break;

				case 'KeySig':
					$mode = ($msg[3]=='major')?0:1;
					$xml .= "<KeySignature Fifths=\"{$msg[2]}\" Mode=\"$mode\"/><BR>"; // ???
					break;
				...
				case 'SeqSpec':
					...
					$xml .= "<SequencerSpecific>$data</SequencerSpecific><BR>";
					break;
				...
				case 'SysEx':
					...
					$xml .= "<SystemExclusive>$str</SystemExclusive><BR>";
					break;
/* TODO:
...
//---------------------------------------------------------------
function playMidFile($file,$visible=true,$autostart=true,$l  oop=true,$player='default'){
		...
		case 'Meta':
			...
			switch ($type){
				...
				case 'Cue': //0x07: // Meta Cue
					$texttypes = array('Text','Copyright','TrkName','InstrName','Ly  ric','Marker','Cue');
...
// if ($trackLen != $trackLen2) { echo "Warning: TrackLength is corrupt ($trackLen != $trackLen2)! <BR>"; }
  $p=4;
  $time = 0;
  $track = array();
  while ($p<$trackLen){
					  ...
					  switch (@$last){	  
						  ...
						  case 'Pb':
							...
							case 'MetaChannelPrefix':
							  ...
							  break;
						  default:
// MM: ToDo: Repetition of SysEx and META-events? with <last>?? <BR>";
